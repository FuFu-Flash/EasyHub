  ...
  return <div className="app-shell live-shell" ref={appRoot}>
    ...
    <div className="main-column" ref={scrollArea}><header className={`topbar topbar-${windowStyle}`}>
      ...
      {view === 'settings' && <>
        ...
        <section className="panel settings-panel"><div className="settings-icon amber"><Info size={22} /></div><div><h2>关于 EasyHub</h2><p>Windows 桌面版 · 直接连接 GitHub</p><span className="settings-version">版本 1.0.0</span><div className="license-details"><strong>GNU GPLv3</strong><span>本应用采用 GNU General Public License 第 3 版。</span><button className="text-link" onClick={() => void window.easyHub?.openLicense()}>查看许可协议 <ArrowRight size={15} /></button></div></div></section></div>
      </>}
