  ...
  return <div className="app-shell live-shell" ref={appRoot}>
    ...
    <div className="main-column" ref={scrollArea}><header className={`topbar topbar-${windowStyle}`}>
      ...
      {view === 'settings' && <>
        ...
        <div className="settings-stack"><AiSettingsPanel language={language} /><section className="panel settings-panel"><div className="settings-icon blue"><Square size={22} /></div><div className="settings-panel-content"><h2>窗口控件</h2><p>选择窗口顶部按钮的外观。更改会立即生效，并保存在这台电脑上。</p><div className="window-style-options" role="group" aria-label="窗口控件样式"><button className={`window-style-option ${windowStyle === 'windows' ? 'selected' : ''}`} aria-pressed={windowStyle === 'windows'} onClick={() => setWindowStyle('windows')}><span className="window-style-preview windows-preview" aria-hidden="true"><span /><span /><span /></span><span><strong>Windows 风格</strong><small>默认 · 右上角按钮</small></span>{windowStyle === 'windows' && <Check size={18} className="window-style-check" />}</button><button className={`window-style-option ${windowStyle === 'reference' ? 'selected' : ''}`} aria-pressed={windowStyle === 'reference'} onClick={() => setWindowStyle('reference')}><span className="window-style-preview reference-preview" aria-hidden="true"><span /><span /><span /></span><span><strong>圆点风格</strong><small>参考图 · 左上角圆点</small></span>{windowStyle === 'reference' && <Check size={18} className="window-style-check" />}</button></div></div></section>
        ...
        <section className="panel settings-panel"><div className="settings-icon amber"><Info size={22} /></div><div><h2>关于 EasyHub</h2><p>Windows 桌面版 · 直接连接 GitHub</p><span className="settings-version">版本 1.2.0</span><div className="license-details"><strong>GNU GPLv3</strong><span>本应用采用 GNU General Public License 第 3 版。</span><button className="text-link" onClick={() => void window.easyHub?.openLicense()}>查看许可协议 <ArrowRight size={15} /></button></div></div></section></div>
      </>}
