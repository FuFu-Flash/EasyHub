...
export function LocalWorkspace({ mode, repos, selectedRepo, onBack, onCreated, onDownloadProject, downloadBusy, initialSyncReview, onSyncReviewOpened }: Props) {
  ...
  return <div className="local-workspace">
    ...
    {visibleLinks.map((item) => { const status = statuses[item.id]; return <section className="panel live-section local-project-card" key={item.id}>
      ...
      <div className="local-actions"><button className="button button-quiet" disabled={busy} onClick={() => void api?.localOpenFolder(item.id)}><FolderOpen size={16} />打开文件夹</button><button className="button button-quiet" disabled={busy} onClick={() => void task(async () => { if (api) { const next = await api.localStatus(item.id); setStatuses((old) => ({ ...old, [item.id]: next })); } })}><RotateCw size={16} />检查修改</button><button className="button button-quiet" disabled={busy} onClick={() => void checkLatest(item)}><RotateCw size={16} />获取最新</button>{status?.files.length ? <button className="button button-primary" disabled={busy} onClick={() => { setPublishId(item.id); setUpdateMessage(''); }}><Send size={16} />发布源码</button> : null}</div>
