  ...
  return <div className="app-shell" ref={appRoot}>
    ...
    <div className="main-column" ref={scrollArea}>
      ...
      <main className="page-content">
        ...
        {route.name === 'new-release' && project && <ReleaseEditor key={project.id} project={project} language={language} busy={Boolean(busy)} imageSources={imageSources} onRegisterInlineImage={(id, file) => setImageSources((current) => ({ ...current, [id]: URL.createObjectURL(file) }))} onPublish={(input) => handlePublishRelease(project.id, input)} onBack={() => navigate({ name: 'project', projectId: project.id })} onOpenUpdate={() => navigate({ name: 'publish', projectId: project.id })} onOpenLink={openReleaseLink} />}
