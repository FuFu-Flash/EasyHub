...
import type { GitHubRepo, GitHubSearchUser, TrendingPage, TrendingPeriod } from '@easyhub/github';
...
const cache = new Map<string, { at: number; result: TrendingPage }>();
...
export function TrendingDiscover({ query, setQuery, scope, onScopeChange, period, onPeriodChange, page, onPageChange, displayMode, onDisplayMode, history, onSelectHistory, onClearHistory, searchResults, searchBusy, searchError, onOpen, onOpenProfile, onSearch, onUserSearched }: { query: string; setQuery: (value: string) => void; scope: DiscoverScope; onScopeChange: (scope: DiscoverScope) => void; period: TrendingPeriod; onPeriodChange: (period: TrendingPeriod) => void; page: number; onPageChange: (page: number) => void; displayMode: SearchDisplayMode; onDisplayMode: (mode: SearchDisplayMode) => void; history: SearchEntry[]; onSelectHistory: (entry: SearchEntry) => void; onClearHistory: () => void; searchResults: GitHubRepo[]; searchBusy: boolean; searchError: string; onOpen: (repo: GitHubRepo) => void; onOpenProfile: (user: GitHubSearchUser) => void; onSearch: (value: string) => void; onUserSearched: (value: string) => void }) {
  const key = `${period}:${page}`;
  const [result, setResult] = useState<{ key: string; value: TrendingPage }>(() => ({ key, value: cache.get(key)?.result ?? { items: [], page, hasNextPage: false } }));
  ...
  useEffect(() => {
    ...
    if (!refresh && cached && Date.now() - cached.at < 600000) { setResult({ key, value: cached.result }); setBusy(false); setError(''); return () => { active = false; }; }
    setBusy(true); setError(''); setResult({ key, value: { items: [], page, hasNextPage: false } });
    void (window.easyHub ? window.easyHub.github<TrendingPage>('trending', period, page) : Promise.reject(new Error('应用连接不可用，请重新启动 EasyHub。'))).then((response) => {
      if (!active) return;
      setResult({ key, value: response });
      cache.set(key, { at: Date.now(), result: response });
    ...
    return () => { active = false; };
  }, [key, page, period, refresh, scope]);
  const current = result.key === key ? result.value : cache.get(key)?.result;
  const visible = query.trim().length >= 2 ? searchResults : current?.items ?? [];
  const projectAddress = parseProjectAddress(query);
  const isSearch = query.trim().length >= 2 && !projectAddress;
  ...
  return <div className="discover-page"><div className="page-header"><div><div className="eyebrow">发现更多作品</div><h1>发现 / 搜索</h1><p>搜索公开项目与 GitHub 用户，也可以看看大家最近在创作什么。</p></div></div>
    ...
    <label className="discover-search"><Search size={21} /><input aria-label={scope === 'users' ? '搜索用户' : '搜索公开项目'} placeholder={scope === 'users' ? '搜索 GitHub 用户…' : '搜索所有公开项目或粘贴项目地址…'} value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && (scope === 'projects' || projectAddress)) onSearch(query); }} /></label>
    ...
    {scope === 'projects' && !hasQuery && <><div className="trending-heading"><div><h2>EasyHub 热门</h2><p>EasyHub 根据 Star 数、近期活动和更新时间排列公开项目。不是 GitHub 官方 Trending。</p></div><button className="text-link" onClick={() => setRefresh((value) => value + 1)}>刷新</button></div><div className="segmented trending-periods" role="group" aria-label="热门时间范围">{periods.map((item) => <button key={item} aria-pressed={period === item} className={period === item ? 'selected' : ''} onClick={() => onPeriodChange(item)}>{labels[item]}</button>)}</div></>}
    {isSearch && <h2 className="discover-results-title">{scope === 'users' ? '用户搜索结果' : '公开项目搜索结果'}</h2>}
    {projectAddress ? <button className="project-address-suggestion" onClick={() => onSearch(query)}><Search size={18} /><span>打开项目 <strong>{projectAddress.owner}/{projectAddress.name}</strong></span><ArrowRight size={18} /></button> : scope === 'users' ? isSearch ? <UserSearchResults query={query} displayMode={displayMode} onOpenProfile={onOpenProfile} onOpenRepository={onOpen} onSearched={onUserSearched} /> : <p className="live-empty">输入至少 2 个字符，搜索 GitHub 用户。</p> : hasQuery && !isSearch ? <p className="live-empty">输入至少 2 个字符，搜索公开项目。</p> : (isSearch ? searchBusy : busy) ? <p className="live-empty"><Clock3 size={16} /> 正在从 GitHub 获取项目…</p> : (isSearch ? searchError : error) ? <p className="live-error" role="alert">{isSearch ? searchError : error}</p> : <div className={`trending-list ${isSearch ? `search-result-grid ${displayMode}` : ''}`}>{visible.map((repo, index) => <button className={`trending-card ${isSearch && displayMode === 'compact' ? 'compact' : ''}`} key={repo.id} onClick={() => onOpen(repo)}><span className="trending-rank">{isSearch ? <Search size={17} /> : `#${(page - 1) * 30 + index + 1}`}</span><span className="trending-author-avatar">{repo.owner.avatar_url ? <img src={repo.owner.avatar_url} alt={`${repo.owner.login} 的头像`} /> : repo.owner.login.slice(0, 1).toUpperCase()}</span><span className="trending-content"><strong>{repo.name}</strong><small>{repo.owner.login}</small>{(!isSearch || displayMode === 'detailed') && <span>{repo.description || '还没有项目简介。'}</span>}<span className="trending-meta">{(!isSearch || displayMode === 'detailed') && <em>{repo.language || '未标注语言'}</em>}<em><Star size={14} />{(repo.stargazers_count ?? 0).toLocaleString()}</em>{(!isSearch || displayMode === 'detailed') && <em><TrendingUp size={14} />{isSearch ? '公开项目' : trend(repo, snapshots, period === 'today' ? 1 : period === 'week' ? 7 : 30)}</em>}</span></span><ArrowRight size={19} /></button>)}{visible.length === 0 && <p className="live-empty">{isSearch ? '没有找到公开项目。' : '暂时没有符合条件的热门项目。'}</p>}</div>}
    {scope === 'projects' && !hasQuery && (page > 1 || current?.hasNextPage) && <nav className="trending-pagination" aria-label="热门项目翻页"><button className="button button-quiet" disabled={page === 1 || busy} onClick={() => onPageChange(page - 1)}>上一页</button><span>第 {page} 页</span><button className="button button-quiet" disabled={!current?.hasNextPage || busy} onClick={() => onPageChange(page + 1)}>下一页</button></nav>}
