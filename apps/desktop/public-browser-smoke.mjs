...
try {
  await app.evaluate(({ ipcMain }) => {
    ...
    ipcMain.handle('easyhub:github', (_event, action, ...args) => {
      ...
      if (action === 'publicRepo') return { ...project, id: 201, name: 'other-repo', full_name: 'other-author/other-repo', owner: { login: 'other-author' } };
  ...
  await browser.getByRole('button', { name: '提出问题' }).click();
  await page.getByRole('dialog', { name: '提出问题' }).getByRole('textbox', { name: '问题标题' }).fill('A new question');
  await page.getByRole('dialog', { name: '提出问题' }).getByRole('button', { name: '创建问题' }).click();
